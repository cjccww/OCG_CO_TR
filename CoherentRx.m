classdef CoherentRx < handle
    ...
    methods
        ...
        % 对复数信号进行整体延迟
        function outSignal = complex_signal_delay(obj,input,delay)
                s_skewed = delay_apply(input, obj.signalPHY.fs, delay);
                outSignal=s_skewed.';
        end

        % 创建二参量——RSOP
        function outSignal = addRsop_two_paramera(obj,input,psp_theta,psp_phi)
            %psp_theta = pi/4;  % 主态方向角度θ
            %psp_phi = pi/4;  % 主态方向角度φ
            ...
            mat_theta = cos(psp_theta) * sig0 ...
                - sin(psp_theta) * sig3i ;  % 正交矩阵  % 不确定是否需要写成复数形式
            ...
            sig_fft_x = mat_rot(1, 1) * input(:,1) + mat_rot(1, 2) * input(:,2);
            sig_fft_y = mat_rot(2, 1) * input(:,1) + mat_rot(2, 2) * input(:,2);
            % 信号输出
            outSignal(:,1)=sig_fft_x(:);
            outSignal(:,2)=sig_fft_y(:);
        ...
        function outSignal=matchFiltering(obj,input)
            ...
            for indMode=1:obj.TxPHY.TxPHY.Nmodes
                ...
                outSignal(:,indMode)=conv(input(:,indMode),obj.signalPHY.hsqrt);
        ...
        % 计算OSNR
        function osnr = calc_osnr(obj,snr)
            % Calculate OSNR

            %   OSNR = 10 * lg (Ps / ( N0 * Br)) = 10 *lg (Eb * Fs / ( N0 * Br))
            %   Ps: Power of signal N0: Power density of noise
            %   Br: Band of reference 0.1nm Eb: Energy of samples Fs: Sample frequency

            fs = obj.signalPHY.fs;
            b_ref = 12.5e9; % Reference bandwidth in Hz
            osnr = 10*log10(fs/b_ref) + snr;
        end
