...
for i  = 1:1
    ...
    for ind=1:length(tauEstVec)
        ...
        switch (TED)
            ...
            case 'gardner'
                zcStrobeIdx = strobeIdx_2sps(3:end) - SPS/2;   % 符号间过零点位置
                prevStrobeIdx = strobeIdx_2sps(1:end-2);     % 前符号位置
                currStrobeIdx = strobeIdx_2sps(3:end);       % 当前符号位置
                
                % 过零点采样 × (前符号 - 当前符号)
                err = real( conj(matchOut(prevStrobeIdx)) .* ...
                    (matchOut(zcStrobeIdx) - matchOut(currStrobeIdx))); % 也是符合公式
            ...
            case 'gardnerNyquist'
                ...
                %zcStrobeIdx = strobeIdx_2sps(2:end) - SPS;   % 符号间过零点位置
                %prevStrobeIdx = strobeIdx_2sps(1:end-1);     % 前符号位置
                %currStrobeIdx = strobeIdx_2sps(2:end);       % 当前符号位置
                ...
                currStrobeIdx = strobeIdx_2sps(3:end);       % 当前符号位置
                % 功率型 Gardner-Nyquist
                err = abs(matchOut(prevStrobeIdx)).^2 .* (abs(matchOut(currStrobeIdx)).^2 - abs(matchOut(zcStrobeIdx)).^2);

                % 近似
                %err =abs(matchOut(prevStrobeIdx)).^2 .*abs(matchOut(currStrobeIdx)).^2;
            ...
            case 'lee'
                ...
                currStrobeIdx = strobeIdx_2sps(2:end);       % 当前符号位置
                % Lee
                err = ((matchOut(prevStrobeIdx))+1j*(matchOut(currStrobeIdx))).*...
                    (conj(matchOut(prevStrobeIdx))+1j*conj(matchOut(currStrobeIdx))); 
            case 'lee-sign'
                prevStrobeIdx = strobeIdx_2sps(1:end-1);     % 前符号位置
                currStrobeIdx = strobeIdx_2sps(2:end);       % 当前符号位置
                % Lee
                err = sign((matchOut(prevStrobeIdx))+1j*(matchOut(currStrobeIdx))).*...
                    sign(conj(matchOut(prevStrobeIdx))+1j*conj(matchOut(currStrobeIdx))); 
            case 'lee-power'
                prevStrobeIdx = strobeIdx_2sps(1:end-1);     % 前符号位置
                currStrobeIdx = strobeIdx_2sps(2:end);       % 当前符号位置
                % Lee
                err = (abs(matchOut(prevStrobeIdx)).^2+1j*abs(matchOut(currStrobeIdx)).^2).^2;
