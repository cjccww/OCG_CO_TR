...
%         TED = 'gardnerNyquist';
        ...
        switch (TED)
            ...
            case 'mm' % Mueller-Muller TED
                ...
                currStrobeIdx = strobeIdx_1sps(2:end);       % 当前符号位置
                % 前符号 × 当前采样 - 当前符号 × 前采样
                err = qamDualPol(1:end-1) .* matchOut(currStrobeIdx) - ...
                    qamDualPol(2:end)   .* matchOut(prevStrobeIdx);
            ...
            case 'gardnerNyquist'
                ...
                currStrobeIdx = strobeIdx_1sps(2:end);       % 当前符号位置
                % 功率型 Gardner-Nyquist
                err = abs(matchOut(zcStrobeIdx)).^2 .* (abs(matchOut(prevStrobeIdx)).^2 - abs(matchOut(currStrobeIdx)).^2);
...
zerox_v=zeros(1,length(Total_average));
slope_v=zeros(1,length(Total_average));
% 提取过零点
for ind=1:length(Total_average)
    [szslope,zero_x]=zero_slope(normTauE,Total_average(:,ind));
    zerox_v(ind)=zero_x;
    slope_v(ind)=szslope;
%     scurve_v(ind)=sqrt(mean(Total_average(:,ind).^2));
end

delta_t=sqrt(mean(zerox_v.^2)); % 单位 UI
delta_K=sqrt(mean(slope_v.^2)); % 单位 1
line=delta_K * (normTauE-delta_t);
jitter_rms=20*log10(delta_t);

% fprintf('峰对峰jitter:%.6f\n', jitter_pp);
fprintf('均方根jitter:%.6f\n', jitter_rms);
% 绘制过零拟合线
plot(normTauE,line)
plot(delta_t, 0, 'ro', 'MarkerSize', 5);
zero_text = sprintf('(%g, %g)', delta_t, 0);
text(delta_t + 0.03, 0 + 0.01, zero_text);
jitter_text = sprintf('jitter=%g', jitter_rms);
text(delta_t + 0.03, 0 - 0.01, jitter_text);
...
figure;
histogram(diff_matrix, 10, 'Normalization', 'count');
title('频数统计');
figure;
histogram(diff_matrix, 10, 'Normalization', 'pdf');
title('概率密度统计');
